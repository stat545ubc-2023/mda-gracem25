...
**Research Question 1:** How does tree species richness differ between neighborhoods in Vancouver?

From Summarizing activities, will complete Task 4. I will compare the counts and proportions of the number of species in each neighborhood using the categorical variables common_name and neighborhood_name. This will be beneficial towards answering my research question, as species richness is a count of the number of species, in this case tree species, found in an area, in this case subsetted by neighborhood.
...
# now to determine number of tree species (tree species richness) by neighbourhood
tree_sp_per_neighborhood <- vancouver_trees %>%
  group_by(neighbourhood_name) %>%
  distinct(common_name) %>%
  count(neighbourhood_name)

tree_sp_per_neighborhood


```

For graphing activities, I will complete
...
# Now we have trunk_thickness added to the dataset.
...
```{r}
# Histogram #1: Bin size of 30

tree_diameter_histogram_bin30 <- vancouver_trees %>%
...
tree_diameter_histogram_bin10 <- vancouver_trees %>%
  ggplot(aes(diameter, color = "lightgreen", fill = "brown")) +
...
outliers <- vancouver_trees %>% filter(diameter > 100)
...
vancouver_trees_minus_outliers <- vancouver_trees %>% filter(diameter < 100)
